...
function getPaneTitles() {
  return atom.workspace
    .getCenter()
    .getPanes()
    .map(pane => pane.getItems().map(item => item.getTitle()))
    .reduce((all, titles) => all.concat(titles), []);
}

function getGuideView() {
  return atom.workspace
    .getPaneItems()
    .find(item => item.getTitle() === 'Welcome Guide');
}
